...
wget -q -O /etc/noobzvpns/config.json https://0ms.dev/mirrors/raw.githubusercontent.com/rickicode/noobzvpns-docker/main/noobz/config.json
...
if [ -n "$PORT_TLS" ] && [[ "$PORT_TLS" =~ ^[0-9]+$ ]]; then
    ...
    echo "PORT_TLS tidak valid atau tidak diatur. Tidak melakukan perubahan pada config.json."
fi
...
if [ -n "$PORT_NTLS" ] && [[ "$PORT_NTLS" =~ ^[0-9]+$ ]]; then
    ...
    echo "PORT_NTLS tidak valid atau tidak diatur. Tidak melakukan perubahan pada config.json."
fi
